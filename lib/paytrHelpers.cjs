...
function getPaytrConfig() {
  ...
  const amountModeRaw = String(process.env.PAYTR_AMOUNT_MODE || 'decimal').trim().toLowerCase();
  const amountMode = amountModeRaw === 'kurus' ? 'kurus' : 'decimal';
...
function resolvePaytrAmountMode(mode) {
  return String(mode || process.env.PAYTR_AMOUNT_MODE || 'decimal').trim().toLowerCase() === 'kurus'
    ? 'kurus'
    : 'decimal';
}
...
function buildDirectUserBasket(items, payableTotal, amountMode = 'decimal') {
...
/** PAYTR_AMOUNT_MODE=decimal|kurus — hash ve formda aynı format */
function formatPaytrPaymentAmount(amount, amountMode = 'decimal') {
  return resolvePaytrAmountMode(amountMode) === 'kurus'
...
function logPaytrOfficialDiff(context, hashDebug, formFields, config) {
  ...
  const diff = {
    ...
    officialDirectApiNotes: {
      payment_amount:
        config.amountMode === 'decimal'
          ? 'Resmi PHP örneği ile uyumlu ondalık format (ör. "125.00")'
          : 'odeme spp kuruş integer modu (ör. "12500")',
      user_basket:
        config.amountMode === 'decimal'
          ? 'Resmi örnek ile uyumlu ondalık fiyat (ör. "125.00")'
          : 'Kuruş string fiyat (hash dışı)',
