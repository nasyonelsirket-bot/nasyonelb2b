...
const OFFICIAL_DIRECT_API_REFERENCE = {
  ...
  hashFieldOrderWithoutPaymentType: [
    'merchant_id',
    'user_ip',
    'merchant_oid',
    'email',
  ...
  hashFieldTypes: {
    ...
    merchant_oid: 'string',
    email: 'string',
    payment_amount: 'string (official sample: "100.99"; odeme spp: kuruş "12500")',
    payment_type: 'string ("card")',
    installment_count: 'string ("0")',
    currency: 'string ("TL")',
    test_mode: 'string ("0"|"1")',
    non_3d: 'string ("0"|"1")',
  },
  notInHash: ['lang', 'client_lang', 'no_installment', 'max_installment', 'user_basket'],
...
function getPaytrConfig() {
  ...
  const paymentTypeMode = resolvePaytrPaymentTypeMode();
  const minimalPayload = resolvePaytrMinimalPayload();
...
/** omit — hash+form yok (test 1) | form_only — yalnız form (test 2) | official — docs birebir (test 3) */
function resolvePaytrPaymentTypeMode(mode) {
  const raw = String(mode ?? process.env.PAYTR_PAYMENT_TYPE_MODE ?? 'omit').trim().toLowerCase();
  if (raw === 'form_only' || raw === 'form-only') return 'form_only';
  if (raw === 'official' || raw === 'both') return 'official';
  return 'omit';
}

function resolvePaytrMinimalPayload(flag) {
  const raw = String(flag ?? process.env.PAYTR_MINIMAL_PAYLOAD ?? '1').trim().toLowerCase();
  return raw === '1' || raw === 'true' || raw === 'yes';
}

function buildDirectPaytrHashFieldOrder(paymentTypeMode = 'official') {
  const order = [
    'merchant_id',
    'user_ip',
    'merchant_oid',
  ...
  if (resolvePaytrPaymentTypeMode(paymentTypeMode) === 'official') {
    order.push('payment_type');
  }
  order.push('installment_count', 'currency', 'test_mode', 'non_3d');
  return order;
}
...
function buildDirectPaytrHashInput({
  ...
  const mode = resolvePaytrPaymentTypeMode(paymentTypeMode);
  ...
  const parts = {
    ...
    installment_count: paytrHashPart(String(installmentCount), 'installment_count'),
  ...
  return { parts, hashStr, hashMessage, fieldOrder, paymentTypeMode: mode };
}
...
function logPaytrOfficialByteDiff(context, hashDebug, formFields, config) {
  ...
  const formHashFieldParity = {
    ...
    installment_count: {
      form: formFields.installment_count,
      hash: hashDebug.parts.installment_count,
      match: formFields.installment_count === hashDebug.parts.installment_count,
    },
    currency: {
  ...
  const diff = {
    ...
    payment_type_mode: config.paymentTypeMode,
    minimal_payload: config.minimalPayload,
    official_payment_type: OFFICIAL_PAYMENT_TYPE,
    hashFieldOrderMatch:
      JSON.stringify(hashDebug.fieldOrder) === JSON.stringify(OFFICIAL_DIRECT_API_REFERENCE.hashFieldOrder),
...
function logPaytrPayload(context, fields, meta = {}) {
  ...
  console.log(`[paytr:${context}] payment_type_mode`, meta.config?.paymentTypeMode ?? '(unknown)');
  console.log(`[paytr:${context}] minimal_payload`, meta.config?.minimalPayload ?? '(unknown)');
  console.log(`[paytr:${context}] final form body exact`, encodedBody);
  console.log(`[paytr:${context}] curl`, buildPaytrCurlCommand(safe));
  console.log(
...
module.exports = {
  ...
  resolvePaytrPaymentTypeMode,
  resolvePaytrMinimalPayload,
