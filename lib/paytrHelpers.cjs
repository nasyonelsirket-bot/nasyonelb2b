...
function getPaytrConfig() {
  ...
  const noInstallmentRaw = String(process.env.PAYTR_NO_INSTALLMENT ?? '0').trim();
  const noInstallment = noInstallmentRaw === '1' ? '1' : '0';
  const maxInstallment = String(process.env.PAYTR_MAX_INSTALLMENT ?? '0').trim() || '0';
...
/** Direct API sepet — tutar farkını satır olarak dengeler */
function buildDirectUserBasket(items, payableTotal) {
  const lines = (Array.isArray(items) ? items : []).map((item) => [
    String(item.name || 'Ürün').slice(0, 200),
    Number(item.price || 0).toFixed(2),
    Number(item.quantity || 1),
...
/** Direct API dokümantasyonu — ondalık TL (125.00 TL → "125.00") */
function formatDirectPaymentAmountDecimal(amount) {
  const n = Math.round(Number(amount) * 100) / 100;
  return n.toFixed(2);
}

/** PayTR — tutar kuruş cinsinden tam sayı (125 TL → "12500") */
...
function createPaytrTokenHash({
  ...
  const hashStr =
    ...
    paytrHashPart(userBasket, 'user_basket') +
    paytrHashPart(noInstallment, 'no_installment') +
    paytrHashPart(maxInstallment, 'max_installment') +
