...
const OFFICIAL_DIRECT_API_REFERENCE = {
  ...
  notInHash: ['lang', 'no_installment', 'max_installment', 'user_basket'],
...
function buildDirectPaytrHashInput({
  ...
  const parts = {
    ...
    payment_type: paytrHashPart(String(paymentType), 'payment_type'),
...
function logPaytrOfficialByteDiff(context, hashDebug, formFields, config) {
  const hashPartTypes = {};
  for (const key of DIRECT_API_HASH_FIELD_ORDER) {
    const val = hashDebug.parts[key];
    hashPartTypes[key] = {
      value: val,
      typeof: typeof val,
      isString: typeof val === 'string',
      charLength: String(val).length,
      matchesOfficialType: typeof val === 'string',
    };
  }
...
function logPaytrPayload(context, fields, meta = {}) {
  ...
  console.log(`[paytr:${context}] post body`, encodedBody);
  console.log(
