...
function getPaytrConfig() {
  ...
  const paymentTypeHash = resolvePaytrPaymentTypeHash(hashMode);
...
/** official modda hash payment_type — varsayılan '' (boş string testi) */
function resolvePaytrPaymentTypeHash(hashMode) {
  if (resolvePaytrHashMode(hashMode) !== 'official') {
    return undefined;
  }
  if (process.env.PAYTR_PAYMENT_TYPE !== undefined) {
    return String(process.env.PAYTR_PAYMENT_TYPE);
  }
  return '';
}
...
function buildDirectPaytrHashInput({
  ...
  if (mode === 'official') {
    parts.payment_type = paytrHashPart(
      paymentType !== undefined ? paymentType : resolvePaytrPaymentTypeHash(mode),
      'payment_type',
      { allowEmpty: true },
    );
...
function logPaytrPayload(context, fields, meta = {}) {
  ...
  console.log(
    `[paytr:${context}] payment_type_hash_config`,
    JSON.stringify(meta.config?.paymentTypeHash ?? '(n/a)'),
  );
