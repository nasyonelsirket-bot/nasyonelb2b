...
const { extractTrendyolRating, enrichProductReviews } = require('./productReviews.cjs');
...
function pickSalePrice(item) {
  ...
  if (variant?.price) {
    const sale = Number(variant.price.salePrice) || 0;
    const list = Number(variant.price.listPrice) || 0;
    return sale > 0 ? sale : list;
  }
...
function mapProductItem(item, index, page, priceDivisor) {
  ...
  const trendyolRating = extractTrendyolRating(item);
  ...
  const base = {
    ...
    trendyolRating: trendyolRating || null,
  };

  return enrichProductReviews(base);
}
