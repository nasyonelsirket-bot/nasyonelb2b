...
function generateOrderPdfBuffer(order) {
  ...
  if (cfg.showCustomer) {
    ...
    const fullAddress = [customer.address, customer.city, customer.district]
      ...
      .join(', ');
    [
      [labels.companyName || 'Ad Soyad', customerName],
      [labels.contactName, customer.contactName || customer.email || '-'],
      [labels.phone, customer.phone],
      [labels.address, fullAddress || customer.address],
    ].forEach(([label, value]) => {
  ...
  if (cfg.showSummary) {
    ...
    const summary = [
      ...
      ...(Number(discount.discountAmount) > 0
        ? [`İndirim: ${discount.tierLabel || ''} (-${formatPrice(discount.discountAmount)})`]
        : []),
