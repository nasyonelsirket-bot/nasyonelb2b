...
function registerFonts(doc) {
  const regularB64 = fs.readFileSync(path.join(FONTS_DIR, 'Roboto-Regular.ttf')).toString('base64');
  const boldB64 = fs.readFileSync(path.join(FONTS_DIR, 'Roboto-Bold.ttf')).toString('base64');
