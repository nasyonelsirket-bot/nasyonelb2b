 ...
 *   payment_type hash değeri: '' (boş string — PAYTR_PAYMENT_TYPE ile override)
 *
 * Form-only (hash dışı): lang, no_installment, max_installment
 *
 * PAYTR_HASH_MODE=official (varsayılan) | compact
 * PAYTR_PAYMENT_TYPE=  (varsayılan boş) | card
 */
const {
  getPaytrConfig,
...
function buildPaytrOdemeForm({
  ...
  if (config.hashMode === 'official') {
    hashParams.paymentType = config.paymentTypeHash;
  }
