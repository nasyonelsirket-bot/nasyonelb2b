 ...
 *
 * PAYTR_AMOUNT_MODE=decimal (varsayılan, resmi örnek) | kurus
 * payment_amount hash ve formda aynı formatta olmalı.
 */
const {
  getPaytrConfig,
...
function buildPaytrOdemeForm({
  ...
  const userBasket = buildDirectUserBasket(items, orderTotal, config.amountMode);
