/**
 * PayTR /odeme — tek kaynak: form alanları + token imzası.
 *
 * PayTR dokümantasyonu:
 * - Token hash: payment_amount kuruş (integer)
 * - POST formu: payment_amount ondalık TL ("69.00")
 * - Hash formülü: Direct API (payment_type, non_3d, installment_count)
 */
const {
  getPaytrConfig,
...
function buildPaytrOdemeForm({
  ...
  const paymentAmountDecimal = formatDirectPaymentAmountDecimal(orderTotal);
  ...
  return {
    ...
    payment_amount: paymentAmountDecimal,
