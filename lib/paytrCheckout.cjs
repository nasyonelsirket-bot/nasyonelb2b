 ...
 *
 * payment_amount: kuruş tam sayı ("12500"). user_basket: JSON, satır fiyatları kuruş.
 * odeme spp: no_installment, max_installment, lang, timeout_limit forma eklenir (hash dışı).
 */
const {
  getPaytrConfig,
...
function buildPaytrOdemeForm({
  ...
  const noInstallment = config.noInstallment === '1' ? '1' : '0';
  const maxInstallment = String(config.maxInstallment ?? '0').trim() || '0';
  ...
  logPaytrPayload(`checkout:${orderId}`, fields, config);
