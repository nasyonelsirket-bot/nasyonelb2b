 ...
 *
 * official hash (varsayılan):
 *   merchant_id + user_ip + merchant_oid + email + payment_amount
 *   + [payment_type] + [installment_count] + currency + test_mode + non_3d + merchant_salt
 *
 * PAYTR_PAYMENT_TYPE_VARIANT=omit|empty|card
 * PAYTR_INSTALLMENT_IN_HASH=1|0
 * PAYTR_DEBUG_FILES=1 — logs/paytr-debug/*.txt
 * npm run paytr:matrix — 6 kombinasyon otomatik test
 */
const {
  getPaytrConfig,
...
const { applyPaymentTypeToFormFields } = require('./paytrHashMatrix.cjs');
...
function buildPaytrOdemeForm({
  ...
  const userBasket = buildDirectUserBasket(items, amountAnalysis.parsedTlAmount, config.basketMode);
  ...
  const hashParams = {
    ...
    paymentTypeVariant: config.paymentTypeVariant,
    installmentInHash: config.installmentInHash,
    installmentCount,
    currency,
    testMode,
    non3d,
