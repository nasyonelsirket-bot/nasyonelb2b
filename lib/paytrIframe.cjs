...
const { analyzePaytrAmount, buildDirectUserBasketDecimal } = require('./paytrHelpers.cjs');
...
function buildIframeUserBasket(items, orderTotalTl) {
  const json = buildDirectUserBasketDecimal(items, orderTotalTl);
...
function buildIframeGetTokenPayload({
  ...
  return {
    ...
    paymentAmount,
    userBasketB64,
    userBasketJson: buildDirectUserBasketDecimal(items, amountAnalysis.parsedTlAmount),
    amountAnalysis,
  };
}
...
function logIframeTokenFlow(context, payload, meta = {}) {
  ...
  console.log(`[paytr:${context}] user_basket json`, payload.userBasketJson);
  console.log(`[paytr:${context}] user_basket base64`, body.user_basket);
...
async function requestPaytrIframeToken(payload, context = 'iframe') {
  ...
  console.log(`[paytr:${context}] iframe token response status`, response.status);
  console.log(`[paytr:${context}] iframe token response raw`, rawText);

  if (json.status !== 'success' || !json.token) {
    const reason = json.reason || json.err_msg || rawText.slice(0, 200) || 'PayTR iFrame token alınamadı';
