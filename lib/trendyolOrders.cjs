...
/** En çok satanlar için Trendyol sipariş penceresi */
const DEFAULT_SALES_PERIOD_DAYS = 15;
...
function aggregatePackages(packages, maps) {
  ...
  (Array.isArray(packages) ? packages : []).forEach((pkg) => {
    ...
    if (SKIP_PACKAGE_STATUS.has(pkgStatus) || isExcludedOrderStatus(pkgStatus)) return;

    (Array.isArray(pkg.lines) ? pkg.lines : []).forEach((line) => {
      const lineStatus = line.orderLineItemStatusName || line.status || '';
      if (SKIP_LINE_STATUS.has(lineStatus) || isExcludedOrderStatus(lineStatus)) return;
...
/**
 * Son N gün Trendyol siparişlerinden barkod/stok kodu bazlı satış adedi (iptal/iade hariç)
 */
async function fetchTrendyolSalesAggregates(credentials, options = {}) {
  const days = Math.min(30, Math.max(7, Number(options.days) || DEFAULT_SALES_PERIOD_DAYS));
  const end = Date.now();
...
function applySalesToProducts(products, salesMaps) {
  return (Array.isArray(products) ? products : []).map((p) => {
    ...
    return {
      ...
      trendyolSalesPeriodDays: salesMaps.periodDays || DEFAULT_SALES_PERIOD_DAYS,
      trendyolSalesSyncedAt: salesMaps.syncedAt || null,
    };
  });
}

module.exports = {
  DEFAULT_SALES_PERIOD_DAYS,
