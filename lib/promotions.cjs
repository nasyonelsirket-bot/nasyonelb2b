...
const DEFAULT_PROMOTIONS = {
  ...
  campaigns: [
    ...
    },
    {
      id: 'camp-bundle',
      type: 'bundle',
      title: 'Birlikte al — özel fiyat',
      description: 'Sepette uyumlu ürün önerisi; minimum adet ve 500 TL üzeri ekstra %5 indirim.',
      active: true,
    },
    {
      id: 'camp-bogo',
...
function normalizePromotions(raw) {
  ...
  return {
    ...
    freeShippingThreshold: 0,
    standardShippingFee: 0,
    highValueDiscountThreshold: Number(p.highValueDiscountThreshold) || 500,
    highValueDiscountPercent: Number(p.highValueDiscountPercent) || 5,
    deliveryReward: {
...
function computeCartTotals({ subtotal, paymentMethod, couponResult, promotions }) {
  ...
  const hvThreshold = Number(promos.highValueDiscountThreshold) || 500;
  const hvPercent = Number(promos.highValueDiscountPercent) || 5;
