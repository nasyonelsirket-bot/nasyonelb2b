const { createBlobStorage, loginMember } = require('../../lib/members.cjs');
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await loginMember(storage, body);
    if (!result.ok) {
      return {
    ...
    return {
      statusCode: 500,
      headers: HEADERS,
      body: JSON.stringify({ error: err.message || 'Giriş başarısız' }),
    };
  }
};
