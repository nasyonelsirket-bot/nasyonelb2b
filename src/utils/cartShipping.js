...
export function getFreeShippingStatus(subtotal, threshold = FREE_SHIPPING_THRESHOLD_TL) {
  ...
  if (amount <= 0) {
    return {
      ...
      progressPercent: 0,
      shippingFee: 0,
      label: 'Kargo ücreti sipariş onayında belirlenir',
      upsellMessage: null,
      successMessage: null,
    };
  }
  ...
  return {
    ...
    progressPercent,
    shippingFee,
    label: eligible ? 'Kargo bedava' : `Kargo ${formatPrice(STANDARD_SHIPPING_FEE_TL)}`,
    upsellMessage: !eligible
      ? `Sepete ${formatPrice(remaining)} daha ürün eklersen kargo bedava! (Şu an ${formatPrice(STANDARD_SHIPPING_FEE_TL)} kargo)`
      : null,
    successMessage: eligible
      ? `${formatPrice(threshold)} üzeri siparişinizde kargo bedava uygulandı.`
      : null,
  };
}
